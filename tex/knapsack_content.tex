...
\begin{itemize}
  ...
  \item \textit{Welokrotnie ograniczony problem plecakowy} - najbardziej ogólny typ, który jest problemem programowania całkowitoliczbowego z dodatnimi współczynnikami:
...
\begin{figure}
  \includegraphics[width=\textwidth]{../image/reduced_chvatal_book_sample.png}%
  \caption{Zredukowane drzewo wyliczeń możliwych rozwiązań}
...
\begin{algorithm}
  \begin{algorithmic}[1]
    \State M := 0
    \State k := 0
    \For {j := k+1 TO m} \label{alg:step2}
      \State $ x_j = \lfloor{(c - \sum_{i=1}^{j-1}w_ix_i)/w_j}\rfloor$
    \EndFor
    \State k := m
    \If {$\sum_{i=1}^m p_ix_i > M$}
      \State M := $\sum_{i=1}^m p_ix_i$
      \For {j := 1 TO m}
        \State $x_j^* = x_j$
      \EndFor
    \EndIf
    \If {$k = 1$} \label{alg:step4}
      \State stop
    \Else
      \State $k=k-1$
    \EndIf
    ...
    \Else
      \State $x_k = x_k - 1$
    \EndIf
    \If {!$\sum_{i=1}^k p_x\bar{x}_i + \frac{p_{k+1}}{w_{k+1}}(c - \sum_{i=1}^kw_i\bar{x}_i) < M + 1$}
